...
public class M_PlayerPush3D : MonoBehaviour
{
    /// <summary>
    /// どの状態の時に押せるようにするか
    /// </summary>
        ...
        switch (mode)
        {
            ...
            case MODE.Back:
                ...
                {                   
                    ...
                    if (PlayerObj.transform.localScale.x == 1.0f)
                    {
                        dir = transform.right;
                    }     
                    else if(PlayerObj.transform.localScale.x == -1.0f)
                    {                       
                        dir = transform.right;
                    }                   

                    push.GetComponent<Rigidbody>().velocity = Vector2.zero;
                    push.GetComponent<Rigidbody>().AddForce(dir * fPower, ForceMode.Impulse);                    
                    push.GetComponent<S_EnemyBall>().SetisPushing(true);                   
