...
public class N_ProjecterSympathy3DK : MonoBehaviour
{
    ...
    // 投影機のスクリプトを格納するリスト
    private List<N_ProjectHologram3DK> list = new List<N_ProjectHologram3DK>();
    ...
    void Start()
    {
        ...
        {
            if(transform.GetChild(i).gameObject.name=="Sound")
            {
                playSound = transform.GetChild(i).gameObject.GetComponent<N_PlaySound>();
                break;
            }
        }        
    }
    ...
    void Update()
    {
        ...
        if (!isInitialized)
        {
            ...
            {
                // リストに入れる
                list.Add(child.GetComponent<N_ProjectHologram3DK>());
                i++;
            }
        ...
        if(Input.GetKeyDown(KeyCode.E) || Input.GetButtonDown("SympathyButton"))
        {
            if(isSympathy == false && !playSound.GetIsPlaying())
            {
                ...
                // リストに格納された各オブジェクト
                foreach (N_ProjectHologram3DK SC_Holo in list)
                {
                    // 初期化
                    SC_Holo.Initialize();
                }
