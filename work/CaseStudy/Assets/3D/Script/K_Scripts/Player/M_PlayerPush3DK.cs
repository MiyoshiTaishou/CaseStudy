...
public class M_PlayerPush3DK : MonoBehaviour
{
    /// <summary>
    /// どの状態の時に押せるようにするか
    /// </summary>
    ...
    void Update()
    {      
        if(!M_GameMaster.GetGamePlay() || DuctManager.GetComponent<M_DuctManager3DK>().ContainsTrueValue())
        {
            return;
        }
        ...
        switch (mode)
        {
            ...
            case MODE.None:
                ...
                if (Input.GetKeyDown(KeyCode.Return) || Input.GetButtonDown("EnemyPush"))
                {
                    ...
                    push.GetComponent<Rigidbody>().AddForce(dir * fPower, ForceMode.Impulse);
                    push.GetComponent<S_EnemyBall3DK>().SetisPushing(true);
                }
