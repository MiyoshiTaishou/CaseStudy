...
public class N_EnemyManager3DK : MonoBehaviour
{
    ...
    public List<SEnemyMove3DK> sEnemyMoves = new List<SEnemyMove3DK>();
    ...
    void PartitionTeam(int _num)
    {
        ...
        N_EnemyManager3DK sc_mana = manager.AddComponent<N_EnemyManager3DK>();
    ...
    public void UnionTeam(N_EnemyManager3DK _manager)
    {
        // 自分の隊列に加える
    ...
    private void Patrol()
    {
        if(ElapsedTime == 0.0f)
        {
            ...
            foreach (var obj in TeamMembers)
            {
                obj.GetComponent<K_EnemyReaction3DK>().AllSetFalse();
            }
        }
    ...
    private void Wait()
    {
        ...
        if (ElapsedWaitTime >= WaitTime)
        {
            ...
            foreach (var obj in TeamMembers)
            {
                obj.GetComponent<K_EnemyReaction3DK>().SetIsSearchHologram(false);
            }
        }
        else
        {
    ...
    private void FoundTarget()
    {
        if(ElapsedFoundTime == 0.0f)
        {
            ...
            foreach (var obj in TeamMembers)
            {
                obj.GetComponent<K_EnemyReaction3DK>().SetIsSearchTarget(true);
            }
        }
        ...
        if(ElapsedFoundTime >= FoundTime)
        {
            ...
            foreach (var obj in TeamMembers)
            {
                obj.GetComponent<K_EnemyReaction3DK>().SetIsSearchTarget(false);
            }
        }
    }
    ...
    private void SetInfomation(bool _isNewMana)
    {
        for(int i = 0; i < iMemberNum; i++)
        {
            SEnemyMove3DK move = sEnemyMoves[i];
    ...
    private void SetEnemyMoveScript()
    {
        ...
        {
            sEnemyMoves.Add(member.GetComponent<SEnemyMove3DK>());
        }
    }
    ...
    public N_EnemyManager3DK.ManagerState GetState()
    {
        return managerState;
    }
